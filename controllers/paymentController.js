...
module.exports.checkout = async (req, res) => {
  try {
    ...
    for (const item of selectedProducts) {
      ...
      const product = await Product.findById(item.product_id).select("price name stock");
      if (!product) {
...
module.exports.momoCallback = async (req, res) => {
  try {
    ...
    console.log("MoMo callback received:", { orderId, resultCode, message, amount, extraData });
    ...
    if (resultCode === 0) { // Thanh toán thành công
      ...
      if (cart) {
        ...
        const remainingProducts = cart.products.filter(cartItem => {
          return !order.products.some(orderItem => 
...
module.exports.validateVoucher = async (req, res) => {
  try {
    ...
    const validation = await validateVoucher(voucherCode, parseFloat(totalAmount), userId);
